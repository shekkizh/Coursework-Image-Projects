...
int main()
{
	...
	float threshold_matrix[mask_size][mask_size];

	for(i=0; i < mask_size; i++)
	{
		for(j=0;j<mask_size; j++)
		{
			threshold_matrix[i][j] = ((weights[i][j]) + 0.5)*255/(mask_size*mask_size);
		}
	}
	...
	for (i=0; i<height; i+=mask_size)
	{
		for (j=0; j<width; j+=mask_size)
		{
			for (k=0; k<mask_size; k++)
			{
				...
				{
					if ((input[i+k][j+l]) > threshold_matrix[k][l])
						output[i+k][j+l] = 255;
	...
	const char output_file[] = "man_dithered.raw";
