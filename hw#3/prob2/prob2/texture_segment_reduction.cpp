...
int main()
{
	...
	for (i=0; i<height*width;i++)
		data_points[i] = new double [10];

	for (i=0;i<height;i++)
	{
		for (j=0;j<width;j++)
		{
			data_points[(i*width + j)][0] = energy[1][i][j];
			data_points[(i*width + j)][1] = energy[2][i][j];
			data_points[(i*width + j)][2] = energy[5][i][j];
			data_points[(i*width + j)][3] = energy[6][i][j];
			data_points[(i*width + j)][4] = energy[7][i][j];
			data_points[(i*width + j)][5] = energy[10][i][j];
			data_points[(i*width + j)][6] = energy[11][i][j];
			data_points[(i*width + j)][7] = energy[12][i][j];
			data_points [i*width + j][8] = weight*i;
			data_points [i*width + j][9] = weight*j;
		}
	}
	double **centroid = 0;
	cluster_output = k_means(data_points, height*width, 10, 5, 0.0001,centroid);
	for (i=0;i<height;i++)
	{
		for (j=0;j<width;j++)
		{
	...
	const char output_file[] = "output2.raw";
