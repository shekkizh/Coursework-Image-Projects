...
#define output_image "apples_efros2.raw"
			...
			else if(i==0 && j!=0)
			{
				...
				for(k=i_start;k<i_start+block_size;k++)
				{
					for(l=j_start;l<j_start+block_size;l++)
					{
						...
						{
							error[0] = output[k-i_start+i][j+l-j_start+index][0] - input[k][l+index][0];
							error[1] = output[k-i_start+i][j+l-j_start+index][1] - input[k][l+index][1];
							error[2] = output[k-i_start+i][j+l-j_start+index][2] - input[k][l+index][2];
							squared_error = error[0]*error[0] + error[1]*error[1] + error[2]*error[2];
							if (index==0)
							{
								min_error = squared_error;
								boundary_index[k-i_start] = j+index;
							}
							else if (squared_error < min_error)
							{
								min_error = squared_error;
								boundary_index[k-i_start] = j+index;
							}
						...
						{
						if (l-j_start+j > boundary_index[k-i_start])
						{
			...
			else if (i!=0 && j==0)
			{
				...
				for(k=i_start;k<i_start+block_size;k++)
				{
					for(l=j_start;l<j_start+block_size;l++)
					{
						for (index = 0; index<border;index++)
						{
							error[0] = output[k-i_start+i+index][l-j_start+j][0] - input[k+index][l][0];
							error[1] = output[k-i_start+i+index][l-j_start+j][1] - input[k+index][l][1];
							error[2] = output[k-i_start+i+index][l-j_start+j][2] - input[k+index][l][2];
							squared_error = error[0]*error[0] + error[1]*error[1] + error[2]*error[2];
							if (index==0)
							{
								min_error = squared_error;
								boundary_index2[l-j_start] = i+index;
							}
							else if (squared_error < min_error)
							{
								min_error = squared_error;
						...
						if ((i+k-i_start)<height && (j+l-j_start)<width)
						{
						if (i+k-i_start > boundary_index2[l-j_start])
						{
						output[i+k-i_start][j+l-j_start][0] = input[k][l][0];
						output[i+k-i_start][j+l-j_start][1] = input[k][l][1];
						output[i+k-i_start][j+l-j_start][2] = input[k][l][2];
			...
			else
			{
				...
				for(k=i_start;k<i_start+block_size;k++)
				{
					for(l=j_start;l<j_start+block_size;l++)
					{
						...
						if ((i+k-i_start)<height && (j+l-j_start)<width)
						{
						if ((k-i_start+i > boundary_index2[l-j_start]) && (l-j_start+j > boundary_index[k-i_start]))
						{
						output[i+k-i_start][j+l-j_start][0] = input[k][l][0];
						output[i+k-i_start][j+l-j_start][1] = input[k][l][1];
						output[i+k-i_start][j+l-j_start][2] = input[k][l][2];
